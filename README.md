...
You can choose whether to execute the pre-processing on **CPU** or **GPU** by setting the option in the corresponding netconfig in [`harpnext-nuscenes.yaml`](https://github.com/SamirAbouHaidar/HARP-NeXt/blob/main/configs/net/harpnext-nuscenes.yaml) and [`harpnext-semantickitti.yaml`](https://github.com/SamirAbouHaidar/HARP-NeXt/blob/main/configs/net/harpnext-semantickitti.yaml) files. Additionally, when evaluating with **GPU**, always set batch_size=1 in in [`main-config.yaml`](https://github.com/SamirAbouHaidar/HARP-NeXt/blob/main/configs/main/main-config.yaml) to ensure correct pre-processing.
...
To train the TinyViM block variant, use the TinyViM netconfig:
```
python main.py \
--net harpnext \
--dataset nuscenes \
--path_dataset /path/to/nuscenes \
--mainconfig ./configs/main/main-config.yaml \
--netconfig ./configs/net/harpnext-nuscenes-tinyvim.yaml \
--log_path ./logs/harpnext-nuscenes-32x480-tinyvim \
--gpu 0 \
--seed 0 \
--fp16
```
...
--log_path ./logs/harpnext-cutmix-semantickitti-64x512-tinyvim \
--gpu 0 \
--seed 0 \
--fp16
```

## Validation (eval-only)
Use `--eval` to run validation only on the validation split (no training).
```
python main.py \
--net harpnext \
--dataset semantic_kitti \
...
--netconfig ./configs/net/harpnext-semantickitti.yaml \
--log_path ./logs/harpnext-cutmix-semantickitti-64x512-retrain \
--checkpoint ./logs/harpnext-cutmix-semantickitti-64x512-retrain \
--gpu 0 \
--seed 0 \
--fp16 \
--restart \
...
## Test Set Dump (SemanticKITTI)
Use `--test` to run inference on the SemanticKITTI test split and dump `.label` files.
```
python main.py \
--net harpnext \
--dataset semantic_kitti \
...
--fp16 \
--test \
--test_output ./logs/harpnext-cutmix-semantickitti-64x512-retrain/test_pred
```
Predictions are written under `sequences/<seq>/predictions/*.label` inside `--test_output`.
