...
class SelectiveScan(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1):
        if selective_scan_cuda is None:
            raise RuntimeError(
                "selective_scan_cuda is required for TinyViM blocks. "
                f"Original import error: {_selective_scan_import_error}"
            )
...
class HARPNeXtTinyViMBlock(nn.Module):
    def __init__(
        ...
        ssm_dt_rank: Any = "auto",
        ssm_act_layer: Callable[..., nn.Module] = nn.SiLU,
        ssm_conv: int = 3,
        ssm_conv_bias: bool = True,
        ssm_drop_rate: float = 0.0,
        ssm_simple_init: bool = False,
        mlp_ratio: float = 4.0,
        mlp_act_layer: Callable[..., nn.Module] = nn.GELU,
        mlp_drop_rate: float = 0.0,
        use_checkpoint: bool = False,
        index: int = 0,
        **kwargs,
